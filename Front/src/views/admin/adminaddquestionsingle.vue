    ...
    <el-main style="height: 100%;">
        ...
        <div>
            ...
            <QuillEditor ref="quillEditRef" :toolbar="toolbarOptions" theme="snow" @focus="onEditfocus"
                @blur="onEditblur" @ready="onEditblur" />
        </div>
        <div>
            <p>试题选项</p>
            <div v-if="isPanduan">
                        ...
                        <el-row style="margin-bottom: 30px;">
                            ...
                            <el-col :span="20">
                                <QuillEditor :ref="i[0]" :toolbar="toolbarOptions" theme="snow"
                                    @ready="() => onEditready1(i[0], index)" @blur="() => onEditready1(i[0], index)"
...
const addsinglequestion = async () => {
    ...
    if (question_imageslist == []) {
        question_imageslist = "0"
    }
    var imgnum = 0
    for (let index = 0; index < Editoption.value.length; index++) {
        const element = xuanX_imageslist[Editoption.value[index]];
        if (element == []) {
            continue
        }
        imgnum += 1
    ...
    // 向后台提交题目
    await axios.post('http://localhost:8000/api/createquestion/singledao', questiondict)
    ElMessage({
        type: 'success',
        message: `导入题库成功`,
    })
